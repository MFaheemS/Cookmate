...
class SearchRecipeActivity : AppCompatActivity() {
    ...
    private fun showCategoryFilterDialog() {
        ...
        val filterInput = dialogView.findViewById<EditText>(R.id.filter_input)
        ...
        btnApply.setOnClickListener {
            val input = filterInput.text.toString().trim()
            if (input.isNotEmpty()) {
                // Parse categories (format: #lunch, #spicy)
                val newCategories = input.split(",")
                    .map { it.trim().removePrefix("#").lowercase() }
                    .filter { it.isNotEmpty() }

                if (newCategories.isNotEmpty()) {
                    // Add new categories to existing ones (not replace)
    ...
    private fun performSearchWithFilters() {
        ...
        // Build URL with both query and categories
